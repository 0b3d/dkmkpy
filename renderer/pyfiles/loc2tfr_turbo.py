...
def save_data(data, layers, label, lat, lon, num_items, pano, writer):
        coordinates = str(lat) + ',' + str(lon)
        pano_image, _id = pano['pano'], pano['id']
...
def render_images(label, layers, locations, num_items, num_threads, size, locations_to_process, writer):
    # Check if dir exists
    ...
    while loc_counter < total_locations:
        start = time.time()
        location = next(loc_iter)
        loc = GSV(location[0], location[1], 1)
        while loc.pano_list == [None] or loc.pano_list == None:
            print("No GSV for this location trying with the next")
            location = next(loc_iter)
            loc = GSV(location[0], location[1], 1)

        pano = {'pano':loc.download("list")[0], 
                'id': loc.pano_list_as_bytes()[0]}

        ntiles += num_items * len(layers)
        loc_counter += 1
        print("Rendering location " + str(loc_counter) + "/" + str(locations_to_process))
        render_location(label, layers, location, num_items, num_threads, size, pano, writer)
        label  += 1
        end = time.time()
        time_elapsed = end - start
        time_to_finish = time_elapsed * (locations_to_process - loc_counter) / 3600
        print("Images rendered and saved: " + str(loc_counter) + "/" + str(locations_to_process))
        print("Time to process a location: {} sec. Time remaining {} hours".format(time_elapsed, time_to_finish))
        print("Current index processed: {}".format(locations.index(location)))
...
if __name__ == "__main__":
    ...
    process = {"train": False, "validation": True, "test": False}
    ...
    initial_point = int(0.03*len(locations))+1
    labels = initial_point
    locations = locations[initial_point:len(locations)]
    ...
    for dataset in datasets:
        if process[dataset] == True:
            ...
            print("Num of locations to render: " + str(locations_to_process))
            print("Num of tiles to render: {}".format(locations_to_process * len(layers) * num_items))
            writer = tf.python_io.TFRecordWriter(filename)
            render_images(labels, layers, locations, num_items, NUM_THREADS, size, locations_to_process, writer)
            writer.close()
            sys.stdout.flush()
        else:
